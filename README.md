...
- 支持配置多个 TG 账号，上传满 3 个视频后自动切换到下一个账号执行后续下载/上传，降低单账号限速风险。
...
| `TG_ROTATE_AFTER_VIDEOS` | 否 | `3` | 每个 TG 账号上传多少个视频后切换到下一个账号 |
...
| `TG_ADMIN_CONFIG_FILE` | 否 | `./tg_mirror_admin.json` | 管理后台保存 TG 账号资料、源频道、目标频道的本地配置文件 |
...
- 添加/更新/删除 TG 账号资料（保存 API ID、API Hash、Session、手机号备注等；添加或删除账号后需重启进程才能建立/关闭对应 Telethon 登录连接）。
- 多账号轮换：启动时会登录后台配置里的账号，处理任务时使用当前账号下载/上传；每成功上传 `TG_ROTATE_AFTER_VIDEOS` 个视频后自动切换到下一个账号。
- 添加/删除源频道，新增源频道会立即参与监听过滤。
- 添加/删除目标频道，后续搬运会发送到当前目标频道列表。
...
后台管理数据会保存到 `TG_ADMIN_CONFIG_FILE`，默认是 `./tg_mirror_admin.json`；该文件可能包含账号资料和 API Hash，已在 `.gitignore` 中排除，请不要公开。
