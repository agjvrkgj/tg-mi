...
async def send_prepared_album(target: int | str, media_items: list[PreparedMedia], caption: str) -> None:
...
async def send_album(messages: Iterable[Message]) -> None:
    ...
    try:
        ...
        for target in managed_targets():
            await asyncio.wait_for(
                send_prepared_album(parse_channel_value(target), prepared_files, caption),
                timeout=settings.upload_timeout,
            )
        inc_stat("sent_album")
